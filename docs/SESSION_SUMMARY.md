...
1. **課金モデルの確定**: 連続 headless `-p` ループが API 従量課金か claude.ai サブスク認証で回るか。
   これ次第で「連続自走を回し続けてよいか」が決まる(`--real` は現状 `--max-sessions`/`--max-cost` 必須)。
2. **実 claude 初回走 (1回)**: stream-json の実イベント・フィールド名(`usage.input_tokens` 等)を実出力で確認。
   パーサは防御的だが、フィールド名が違えば調整(`parse_stream_json` の数行)。
3. **v2**: L1 制御プレーン(`ctl/schema`+`gate`)をループに結線し rotate/inject を**監査・HITL ゲート**化 /
   token 級ストリーミング表示 / GUI に Approve ボタン。
