...
**空欄パススルー**: 入力欄が**空**のときだけ、plain 矢印/Enter は claude へ直接転送される。
claude の選択 UI (初回の信頼確認・メニュー・モデル選択) はこれで操作する。
1 文字でも入力すると上表のローカル編集規則 (R12/R13) に戻る — 複数行編集中の誤送信は起きない。

**端末クエリ応答の自動転送**: claude が発する端末能力クエリ (DA1 / OSC 11 等) への実端末の
応答は、キー入力と分離して claude へそのまま転送される (入力欄には混入しない)。

## 表示言語 (i18n)

UI 文字列 (GUI ラベル / tooltip / CLI エラーメッセージ) は軽量メッセージテーブル
(`llterm.i18n`) で解決される。表示 locale の解決順:

1. 環境変数 `LLTERM_LANG` (例: `ja` / `en`)
2. OS locale の自動検出
3. 既定 `ja`

zh / ko はメッセージテーブルへの追加だけで後から対応できる (構造は準備済み)。
ループ駆動が Claude へ送る指示プロンプトは **意図的に i18n 対象外** (ユーザー向け表示
ではなくエージェントへの指示文のため、表示 locale で挙動を変えない)。
